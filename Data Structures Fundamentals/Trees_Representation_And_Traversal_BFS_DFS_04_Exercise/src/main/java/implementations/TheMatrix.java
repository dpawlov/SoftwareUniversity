...
public class TheMatrix {
    ...
    public void solve() {
        ...
        while (!coordinates.isEmpty()) {
            ...
            if (isInOfBounds(row + 1, col) && this.matrix[row + 1][col] == this.fillChar) {
                coordinates.offer(new int[] {row + 1, col});
            }
            if (isInOfBounds(row + - 1, col) && this.matrix[row - 1][col] == this.fillChar) {
                coordinates.offer(new int[] {row - 1, col});
            }
            if (isInOfBounds(row, col + 1) && this.matrix[row][col + 1] == this.fillChar) {
    ...
    private void fillMatrix(int row, int col) {
        ...
        if (isOutOfBounds(row, col) || this.matrix[row][col] != this.fillChar) {
            return;
        }
