...
public class ConnectedAreasInMatrix_06 {
    ...
    static class ConnectedArea implements Comparable<ConnectedArea> {
        ...
        private void traverseArea(int row, int column, ConnectedArea area, char[][] matrix) {
        ...
        private boolean isNotInBounds(int row, int column, char[][] matrix) {
            return row < 0 || row >= matrix.length || column < 0 || column >= matrix[row].length;
        }


        private void fillMatrix(char[][] matrix, Scanner scanner) {
            for (int row = 0; row < matrix.length; row++) {
